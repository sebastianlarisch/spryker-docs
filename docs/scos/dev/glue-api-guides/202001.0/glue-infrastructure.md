...
| `getUser` | `getUser(): ?UserInterface` | Get the user associated with the request. |This method is deprecated in favor of `getRestUser`. |
| `getRestUser` | `getRestUser(): ?RestUserTransfer` | Get REST user associated with the request. |This method will return `null` in case the request is performed by guest customer. |
...
| `setPayload` | `setPayload(?AbstractTransfer $payload): self` | Sets resource's payload. | Payload can be used to store the data received from the client call. This data can be later used to build relationships. |
